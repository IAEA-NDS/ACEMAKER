...
C                                *Modified by DLA:
C                                  1) Original LSSF saved on C2 field
C                                  2) Extrapolation formalism in the
C                                     URR modified
C                                     1-A/E --> C/sqrt(1+A/E) for total
C                                               1/sqrt(1+A/E) for g,f
...
C-----08/08/2012 DEFINE CODE NAME
     ...
     1 NBNEED,MYSIGMA0
      ...
      COMMON/SIGPT/epot(3000),spot(3000),ipot
      INCLUDE 'groupie.h'
...
c-----------------------------------------------------------------------
   30 IF(MFH.NE.2.OR.MTH.NE.151) GO TO 40
      CALL FILE2
c      write(55,*)' Potential scattering SPOT'
c      do kipot=1,ipot
c        write(55,*)kipot,epot(kipot),spot(kipot)
c      enddo
      GO TO 20
c-----------------------------------------------------------------------
C
...
C=======================================================================
      INCLUDE 'implicit.h'
      parameter (napmax=500,third=1.0d0/3.0d0)
      CHARACTER*4 FMTHOL
      INTEGER*4 OUTP,OTAPE
      COMMON/HEADER/C1H,C2H,L1H,L2H,N1H,N2H,MATH,MFH,MTH,NOSEQ
      ...
      COMMON/HOLFMT/FMTHOL
      COMMON/SIGPT/epot(3000),spot(3000),ipot
      dimension nbtapes(20),ibtapes(20),eapes(napmax),apes(napmax)
C-----Initialize
      INPART= 1     ! Initialize to neutron (in case not ENDF6 format)
      RES1  = 0.0d0
      RES2  = 0.0d0
...
c
c     2022/11/06 - added Potential scattering calculation for URR
c                  extrapolation by D.L. Aldama (DLA)
c
c-----------------------------------------------------------------------
      NRO = N1H
      if(NRO.eq.1) then
c----- Energy dependent scattering radius = copy TAB1 record
       ...
       do I=1,N2,3                  ! Scattering radius (X,Y) Pairs
        CALL COPY1G
       enddo
c----- Energy dependent scattering radius stored
       if (NIS.eq.1) then
        do i=1,N2,3                  ! back
          backspace(itape)
        enddo
        ...
        enddo
        nrapes=N1
        read(itape,'(6I11)')(nbtapes(i),ibtapes(i),i=1,nrapes)
        npapes=N2
        read(itape,'(6E11.0)')(eapes(i),apes(i),i=1,npapes)
       endif
      endif
...
C
C     LRU= 1 - RESOLVED
C     LRF= 1 - SLBW,        = 2 - MLBW,      = 3 - LIMITED REICH-MOORE,
C        = 4 - ADLER-ADLER, = 7 - NEW REICH-MOORE
C
C     LRU= 2 - UNRESOLVED
...
C-----calculate potential cross section SPOT
      if (IS.eq.1) then
        if(NIS.gt.1) then
          aap=0.1234*(ATWT**third)+0.08
          nnro=0
        else
          if (LRF.eq.3.and.C2H.eq.0.0d0) then
            read(itape,'(11x,E11.0)')C2H
            backspace(itape)
          endif
          aap=C2H
          nnro=NRO
        endif
        call sigpot(EL,EH,nnro,NLS,aap,nrapes,nbtapes,ibtapes,
     &              npapes,eapes,apes,ipot,epot,spot)
      endif
...
C-----------------------------------------------------------------------
  ...
  230 FORMAT(1x,78('=')/
     ...
     1 '         Execution Terminated.'/1x,78('='))
      ...
      END
C=======================================================================
      subroutine sigpot(EL,EH,NRO,NLS,api,nrapes,nbtapes,ibtapes,
     &  npapes,eapes,apes,ipot,epot,spot)
      INCLUDE 'implicit.h'
      parameter (r=1.00461579005238d0,eemin0=4.0d0)
      dimension nbtapes(*),ibtapes(*),eapes(*),apes(*),epot(*),spot(*)
      COMMON/WHATZA/ATWT,IZA,MATNOW,MFNOW
      if (NRO.ne.1) then
        nrapes=1
        nbtapes(1)=2
      ...
      if (EH.gt.eemin) then
        ...
        do while (ee.lt.EH)
          if (ee.ge.eemin) then
            ap=fvalue(nrapes,nbtapes,ibtapes,npapes,eapes,apes,ee)
            ipot=ipot+1
            epot(ipot)=ee
            spot(ipot)=sigpE(ee,ap,MLS,ATWT)
          endif
          ee=ee*r
        enddo
        ap=fvalue(nrapes,nbtapes,ibtapes,npapes,eapes,apes,EH)
        ipot=ipot+1
        epot(ipot)=EH
        spot(ipot)=sigpE(EH,ap,MLS,ATWT)
      endif
      return
      end
C=====================================================================
      real*8 function sigpE(ee,ap,n,atwt)
      implicit real*8 (a-h, o-z)
      parameter (cl0=455.2059793d0, rk0=1.0d0/cl0)
      parameter (pi4=1.25663706143592d+1)
      rk=rk0*atwt/(atwt+1.0d0)*sqrt(ee)
      rho=rk*ap
      sum=0.0d0
      do l=1,n
       ll=l-1
       ...
       elseif (ll.eq.1) then
         phi=rho-atan(rho)
       elseif (ll.eq.2) then
         phi=rho-atan(3.0d0*rho/(3.0d0-rho*rho))
       elseif (ll.eq.3) then
         rho15=(15.0d0-rho)
         phi=rho-atan(rho*rho15*rho15/(15.0d0-6.0d0*rho*rho))
       endif
       sum=sum+(2*ll+1)*sin(phi)*sin(phi)/(rk*rk)
      enddo
      sigpE=pi4*sum
      return
      end
...
c      Extrapolate outside the range as a constant value equal to
c      y(1) or y(np)
c
        implicit real*8 (a-h, o-z)
...
c-----2019/6/8 - Only allow for TART 616 groups
      if(NGR.eq.616) then
c-----2019/6/23 - Only allow for standard Sigma0 definition
...
C=======================================================================
      ...
      URES1=edelta(URES1,1.0d0)
      open(55,file='urrdata.txt')
      write(55,*)' URR data log file'
      write(55,*)' URES1=',URES1,' URES2=',URES2
...
C     3) Partially Shielded: Sigma0 = Unshielded Total Group Average
C
C=======================================================================
      INCLUDE 'implicit.h'
      INCLUDE 'groupie.h'
...
c
c      2022/11/06 Extrapolation formalism modified by DLA:
c                  a) 1-A/E --> 1/sqrt(1+A(x)/E) for (n,tot),(n,g),(n,f)
c                  b) scattering shielding factors are computed from
c                     the total, capture and fission shielded cross 
c                     sections
C                            
C=======================================================================
      INCLUDE 'implicit.h'
      INCLUDE 'groupie.h'
      INTEGER*4 OTAPE2
      parameter (cl0=455.2059793d0, pi4=1.25663706143592d+1)
      COMMON/UNITS/OTAPE2,NTAPE,LIST1,LIST2,LIST3,IPLOT
      common/fitcom/xcmom(3,5,maxgroup),AE2(5),AE3(5)
      common/newmom/xcnew(3,5,maxgroup),NURR1,NURR2
      COMMON/TRASH/ERROK,ER10PC,MGROUP,METHODB,NBAND,NBMAX,
...
c-----------------------------------------------------------------------
      ...
      DO 10 igr=1,NGR
        eg1=EGROUP(igr)
        eg2=EGROUP(igr+1)
        ave=2.0d0*eg1*eg2/(eg1+eg2)
        if(ave.lt.elow ) go to 10
        if(ave.gt.ehigh) then
           igr2=igr
           go to 20
        endif
...
c-----Fit is below URR
      ...
      e2 = 0.9d0*elow
      e3 = 1.5d0*ehigh
c-----Estimate average potential cross section
      write(55,*)
      write(55,*)' Group average potential'
      write(55,*)' -----------------------'
      write(55,'(5a)')' igr ','  Lower Energy ','  Upper Energy ',
     &  ' Average Energy','  Potential XS ' 
      do igr=1,NGR
        eg1=EGROUP(igr)
        eg2=EGROUP(igr+1)
        ave=2.0d0*eg1*eg2/(eg1+eg2)
        if(ave.ge.e1.and.ave.le.e3) then
          avspot(igr)=avepot(eg1,eg2,ipot,epot,spot)
          write(55,'(i4,1p4e15.6)')igr,eg1,eg2,ave,avspot(igr)
        else
          avspot(igr)=0.0d0
        endif
      enddo
c-----WARNING: ir=1 is SPECTRUM: 2-5 = total, elastic, capture, fission
      DO ir = 2,5
       if (ir.ne.3) then
        write(55,*)
        write(55,*)' ===============================================  '
        write(55,*)' RRR for Reaction IR= ',ir
        write(55,*)' ===============================================  '
        n23=0
        suma2=0.0d0
        sumb2=0.0d0
        ...
        do igr=1,NGR
          ...
          eg2=EGROUP(igr+1)
          ave=2.0d0*eg1*eg2/(eg1+eg2)
          if(ave.ge.e1.and.ave.le.e2) then
           n23=n23+1
           spotg=avspot(igr)
           siginf=XCALL(1,ir,igr)
           sigti=XCALL(1,2,igr)
           sum1e=sum1e+siginf/ave
           sume2=sume2+1.0d0/ave
           sigb2=0.0d0
           sigb3=sigti
           write(55,'(1x,2i4,5(a,1pe13.6))')n23,igr,' ave=',ave,
     &      ' siginf=',siginf,' spot=',spotg,' sigb2=',sigb2,
     &      ' sigb3=',sigb3
c--------- sig0=0.0
           ff=xcmom(2,ir,igr)
           a2g=ave*(1.0d0/(ff*ff)-1.0d0)         
           suma2=suma2+ff*siginf/ave
           sig02=sigb2+spotg
           sump2=sump2+spotg/sig02
           sumf2=sumf2+1.0d0/sig02           
           write(55,'(1x,a,1p,2(a,e13.6))')' sigb2 ',' f=',ff,' A=',a2g
c--------- sig0=totinf
           ff=xcmom(3,ir,igr)
           a3g=ave*(1.0d0/(ff*ff)-1.0d0)
           suma3=suma3+ff*siginf/ave
           sig03=sigb3+spotg
           sump3=sump3+spotg/sig03
           sumf3=sumf3+1.0d0/sig03           
           write(55,'(1x,a,1p,2(a,e13.6))')' sigb3 ',' f=',ff,' A=',a3g
           write(55,*)
          elseif (ave.gt.e2) then
            exit
          endif
        enddo
c-----------------------------------------------------------------------
c
c     Define fit: F= 1.0/sqrt(1+A(x)/E)
c
c     x is extrapolated from the RRR by averaging resonance parameters
c     from the upper RRR region and taking into account TART-616 energy
c     structure (constant lethargy intervals)
c           
c     Shielding factors for the elastic scattering are computed from the
c     total, capture and fission extrapolated cross sections in the URR                
c
c-----------------------------------------------------------------------
c-----Test for no points in E range = set to NO Shielding
        if(n23.le.0) then
          AE2(ir) = 0.0d0
          AE3(ir) = 0.0d0
          write(55,*)' NO shielding in the URR for IR=',ir
          write(55,*)' -------------------------------'
        else
          xn23=float(n23)
          xlamb=cl0*(ATWT+1.0d0)/ATWT
          a0=pi4*xlamb*xlamb
          ave=xn23/sume2
          spot2=sump2/sumf2
          rxy2=sump2/xn23          
          if (abs(suma2).gt.1.0d-12) then
            finv=sum1e/suma2
            finv2=finv*finv
            a2g=ave*(finv2-1.0d0)
            x2=0.5d0-sqrt(0.25d0-a2g*spot2/a0)
            AE2(ir)=x2/rxy2
          else
            a2g=0.0d0
            x2=0.0d0
          ...
          spot3=sump3/sumf3
          rxy3=sump3/xn23
          if (abs(suma3).gt.1.0d-12) then          
            finv=sum1e/suma3
            finv2=finv*finv
            a3g=ave*(finv2-1.0d0)
            x3=0.5d0-sqrt(0.25d0-a3g*spot3/a0)
            AE3(ir)=x3/rxy3
          else
            a3g=0.0d0
            x3=0.0d0
          ...
          write(55,*)' URR extrapolation parameters for IR=',ir
          write(55,*)' ------------------------------------'
          write(55,'(a,1p6e13.6)')
     &    ' ave a2g spot2 rxy2 x2 z2',ave,a2g,spot2,rxy2,x2,AE2(ir)
          write(55,'(a,1p6e13.6)')
     &    ' ave a3g spot3 rxy3 x3 z3',ave,a3g,spot3,rxy3,x3,AE3(ir)
        endif
        write(55,*)
        write(55,'(6a)')' igr ',' Average Energy','   F(infinity) ',
     &   '    F(sigb2)   ','    F(sigb3)   ','  Unshielded XS'
...
c-----    define up to e3 = 1.5*ehigh for graphics output only
          ...
          xcnew(2,ir,igr)=furr(ir,ATWT,ave,spotg,sigb2,AE2(ir))
          xcnew(3,ir,igr)=furr(ir,ATWT,ave,spotg,sigb3,AE3(ir))
          if (xcnew(2,ir,igr).gt.xcnew(3,ir,igr))
     &      xcnew(2,ir,igr)=0.999999d0*xcnew(3,ir,igr)
          if (xcnew(2,ir,igr).gt.1.0d0) xcnew(2,ir,igr)=1.0d0
          if (xcnew(3,ir,igr).gt.1.0d0) xcnew(3,ir,igr)=1.0d0
          if (igr.ge.NURR1.and.igr.le.NURR2) then
            write(55,'(i4,1p5e15.6,a)')igr,ave,xcnew(1,ir,igr),
     &        xcnew(2,ir,igr),xcnew(3,ir,igr),xcall(1,ir,igr),'  URR'
          else
            write(55,'(i4,1p5e15.6)')igr,ave,xcnew(1,ir,igr),
     &        xcnew(2,ir,igr),xcnew(3,ir,igr),xcall(1,ir,igr)
          endif
   ...
   60   continue
      ...
      write(55,*)
      write(55,*)' =============================================== '
      write(55,*)' Reaction IR=3 (elastic scattering)'
      write(55,*)' =============================================== '
      write(55,*)' Factors computed from the total, capture and fission'
      write(55,*)
      write(55,'(7a)')' igr ',' Average Energy','   F(infinity) ',
     & '    F(sigb2)   ','    F(sigb3)   ','  Unshielded XS',
     & '  Potential XS '      
      do igr=1,NGR
        eg1=EGROUP(igr)
        ...
        if(ave.ge.e1.and.ave.le.e3) then
          xcnew(1,3,igr) = 1.0d0
          sigsi=XCALL(1,3,igr)
          if (sigsi.gt.0.0d0) then
            siggi=XCALL(1,4,igr)
            sigfi=XCALL(1,5,igr)
            spotg=avspot(igr)
            sigtx=sigsi+siggi+sigfi
            xcnew(2,3,igr)=(xcnew(2,2,igr)*sigtx-xcnew(2,4,igr)*siggi-
     &                      xcnew(2,5,igr)*sigfi)/sigsi
            xcnew(3,3,igr)=(xcnew(3,2,igr)*sigtx-xcnew(3,4,igr)*siggi-
     &                      xcnew(3,5,igr)*sigfi)/sigsi
            if (xcnew(2,3,igr).gt.xcnew(3,3,igr))
     &        xcnew(2,3,igr)=0.999999d0*xcnew(3,3,igr)
            if (xcnew(2,3,igr).gt.1.0d0) xcnew(2,3,igr)=1.0d0
            if (xcnew(3,3,igr).gt.1.0d0) xcnew(3,3,igr)=1.0d0
          else
            xcnew(2,3,igr) = 1.0d0
            xcnew(3,3,igr) = 1.0d0
          endif
          if (igr.ge.NURR1.and.igr.le.NURR2) then
            write(55,'(i4,1p6e15.6,a)')igr,ave,xcnew(1,3,igr),
     &        xcnew(2,3,igr),xcnew(3,3,igr),sigsi,spotg,'  URR'
          else
            write(55,'(i4,1p6e15.6)')igr,ave,xcnew(1,3,igr),
     &        xcnew(2,3,igr),xcnew(3,3,igr),sigsi,spotg
          endif          
        endif
      enddo
      RETURN
      END
c=======================================================================
      real*8 function avepot(E1,E2,ipot,epot,spot)
      implicit real*8 (a-h, o-z)
	    dimension epot(*),spot(*)
	    dimension nbt(2),ibt(2)
	    nr=1
	    nbt(1)=ipot
	    ibt(1)=2
	    sum=0.0d0
	    e0=E1
	    y0=fvalue(nr,nbt,ibt,ipot,epot,spot,E1)/E1
	    i0=0
	    do i=1,ipot
	      ee=epot(i)
	      if (ee.gt.E1.and.ee.lt.E2) then
	        ye=spot(i)/ee
	        sum=sum+0.5d0*(ee-e0)*(ye+y0)
	        e0=ee
	        y0=ye
	        i0=i
	      elseif (ee.ge.E2) then
	        exit
	      endif
	    enddo
	    if (e0.lt.E2) then
	      ye=fvalue(nr,nbt,ibt,ipot,epot,spot,E2)/E2
	      sum=sum+0.5d0*(E2-e0)*(ye+y0)
	    endif
	    avepot=sum/log(E2/E1)
	    return
	    end
C=======================================================================	    
	    real*8 function furr(ir,ATWT,ave,spotg,sigb,y)
	    implicit real*8 (a-h, o-z)
	    parameter (cl0=455.2059793d0, pi4=1.25663706143592d+1)
      xlamb=cl0*(ATWT+1.0d0)/ATWT    
	    a0=pi4*xlamb*xlamb/spotg
	    x=y*spotg/(sigb+spotg)
	    a=a0*x*(1.0d0-x)
	    furr=1.0d0/sqrt(1.0d0+a/ave)
	    return
	    end
c=======================================================================
      SUBROUTINE PLOTIT
...
c=======================================================================
      ...
      data plot24/'ZA000000.PLOT.CUR       '/,nplt/56/
c                    zzzaaa
...
c-----------------------------------------------------------------------
      ...
      ii=index(plot24,'.CUR',.true.)
      plot25=plot24(1:ii)//'PLT'
      OPEN(nplt,FILE=plot25,STATUS='UNKNOWN')
      data reacts/
     1 '        ',
     2 'Total   ',
     3 'Elastic ',
     4 'Capture ',
     5 'Fission '/
      if(e1.le.0.0d0.and.e2.le.0.0d0) go to 75
      ncur=2*3
      write(nplt,'(1p,4e11.4,2i11,0p,f4.2)')0.0,13.5,0.0,10.0,1,1,1.5
      write(nplt,'(6i11,i4)')ncur,0,1,0,0,0,0
      write(nplt,'(a15,25x,a3)')'Incident Energy','eV'
      write(nplt,'(a)')'Self-shielding factors'
c-----Loop over reactions: total, elastic, capture, fission
      DO ir = 2,NSECT
c-----Loop over 3 moments to extrapolate
      write(nplt,'(10a1,1x,a8)')ZABCD,reacts(ir)
      write(nplt,'(a)') 
     &  'F1(sig0=infinity), F2(sig0=0), F3(sig0=total_unshielded)'
...
c
c-----------------------------------------------------------------------
      write(IPLOT,10)ZABCD,'F',m,reacts(ir)
   10 format(10a1,1x,a1,i1,1x,a8)
      do 30 igr=1,NGR
      ave=2.0d0*EGROUP(igr)*EGROUP(igr+1)/(EGROUP(igr)+EGROUP(igr+1))
...
c-----Loop over reactions: total, elastic, capture, fission
      do ir=2,NSECT
      write(nplt,'(10a1,1x,a8)')ZABCD,reacts(ir)
      write(nplt,'(a)')'Unshielded and fully-shielded cross sections'
      write(nplt,'(22x,4i11)')0,0,0,0
      write(nplt,'(22x,4i11)')0,0,0,0      
c-----Unshielded
...
c
c-----------------------------------------------------------------------
        xs1=xs0*xcnew(2,2,igr)
        xs2=xs0*xcnew(3,2,igr)
        write(55,'(1x,2(a,1pe12.5))')' TOT = ',xs0,' at sig0 = ',c0
        write(55,'(1x,2(a,1pe12.5))')' TOT = ',xs1,' at sig0 = ',c1
        write(55,'(1x,2(a,1pe12.5))')' TOT = ',xs2,' at sig0 = ',c2
        if (xs0.le.0.0d0.or.xs1.gt.0.999d0*xs0) then
c
c         No self-shielding
c
          aa=xs0
          ...
          factor=0.0d0
          icase=1
        elseif (xs1.gt.0.999d0*xs2) then
c
c         (xs2-xs1) -> 0.0
c
          aa=xs0
          bb=sqrt((xs0+c1)*(xs0-xs1))
          dd=0.0d0
          factor=0.5d0*sqrt((xs0+c1)/(xs0-xs1))
          icase=2
        else
c
c         General self-shielding
c
          aa=0.5d0*(xs0+(c2*(xs0-xs2)-c1*(xs0-xs1))/(xs2-xs1))
          vv=(xs1*c2*(xs0-xs2)-xs2*c1*(xs0-xs1))/(xs2-xs1)
          aa2=aa*aa
          if (aa2.gt.vv) then
            if (abs(vv).gt.1.0d-6*aa2) then
c
c             General case
c
              bb=sqrt(aa2-vv)
...
c
c             Small vv:  xs1*c2(xs0-xs2) - xs2*c1*(xs0-xs1) -> 0.0
c
              bb=aa+vv/(sqrt(aa2-vv)-aa)
              icase=4
            endif
            dd=0.5d0*(aa-xs0)/bb
            factor=0.5d0*(2.0d0*aa+c1-xs0)/bb
          else
            aa=xs0
            bb=0.0d0
            dd=0.0d0
        ...
        BANDT1 = aa-bb
        BANDT2 = aa+bb
        write(55,'(1x,a,i2,4(a,1pe12.5))')' icase=',icase,' a=',aa,
     &  ' b=',bb,' d=',dd,' factor=',factor
        write(55,'(1x,4(a,1pe12.5))')' wt1=',WT1,' wt2=',WT2,
     &  ' T1=',BANDT1,' T2=',BANDT2
        WTBAND  (1,igr) = WT1
        WTBAND  (2,igr) = WT2
...
c-----  Loop over other reactons: elastic, capture, fission
        DO ir=3,5 ! note 3 to 5 (not total)
          xs0= XCALL(1,ir,igr)
          xs1= xs0*xcnew(2,ir,igr)
          write(55,'(1x,a,i2,2(a,1pe12.5))')' IR=',ir,' xs0=',xs0,
     &     ' at sig0 = ',c0
          write(55,'(1x,a,i2,2(a,1pe12.5))')' IR=',ir,' xs1=',xs1,
     &     ' at sig0 = ',c1
          if(xs0.le.xs1) then
            cc=0.0d0
          else
          ...
          write(55,'(1x,a,1pe12.5)')' c=',cc
          if (xs0.le.0.0d0) then
            BANDP1=0.0d0
            BANDP2=0.0d0
          else
            BANDP1=xs0-cc/WT1
            if (BANDP1.lt.0.0d0) then
              if (abs(BANDP1).gt.0.01d0*xs0) then
                write(55,'(1x,a,1pe12.5,a)')' Too negative BAND1 =',
     &            BANDP1,' > 0.01*Unshielded'
              else
                write(55,'(1x,a,1pe12.5,a)')' Negative BAND1 =',BANDP1              
              endif
              BANDP1=1.0d-10
              cc=(xs0-BANDP1)*WT1
              xs1=xs0-cc/factor
              write(55,'(1x,3(a,1pe12.5))')' Corrected values: cc=',
     &          cc,' xs1=',xs1,' at sig0=',c1
            endif
            BANDP2=xs0+cc/WT2
          endif
          write(55,'(1x,2(a,1pe12.5))')' BANDP1=',BANDP1,
     &    ' BANDP2=',BANDP2
c
...
c-----c2 stores original LSSF value
      c2=float(LSSF)
      LSSF   = 0 ! Always use LSSF=0 to force only cross section -
...
c-----c2  = 0.0 or 1.0       ! stored original LSSF value (see above)
      l1  = 0
      l2  = 0
      n1  = 0
      n2  = 0
...
c=======================================================================
      ...
      INCLUDE 'groupie.h'
      common/ACECOM/nunr,NUNR1,NUNR2
c-----Define 2 sigma0 values = infinity and 0
      dimension sigz(2)
      data nsigz/2/
